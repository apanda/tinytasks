...
in compute clusters into \textit{tiny tasks} that each complete in hundreds of milliseconds.
Tiny tasks avoid the need for complex skew mitigation techniques: by
breaking a large job into millions of tiny tasks, work will
be evenly spread over available resources by the scheduler.
%Furthermore, tiny tasks alleviate long wait times seen in today's
%clusters for interactive jobs, since even tasks for batch jobs
%complete quickly.
Furthermore, tiny tasks alleviate long wait times seen in today's
clusters for interactive jobs: even large batch jobs can be split into small
tasks that each complete quickly.
% Maybe remove this sentence?
% Thus, tiny tasks allow for increased utilization without sacrificing
% responsiveness or fairness.
%We demonstrate that using smaller tasks can improve response times by a factor of 5.2.
We demonstrate a 5.2x improvement in response times due to the use of smaller tasks.
...
Recent research has addressed many of these bottlenecks; we
discuss remaining challenges and propose a task execution
framework that can efficiently support tiny tasks.
