...
As demonstrated in Figure~\ref{fig:sparkskew}, while existing frameworks like Spark
benefit from using smaller tasks, we believe there is need for a new task
execution framework to support tiny tasks across all the jobs in a datacenter.
Supporting tiny tasks requires a highly scalable scheduler that can handle hundreds 
of thousands of scheduling decisions per second, and we argue that task launch overheads 
need to be reduced by two orders of magnitude to around $100\mu$s. To
efficiently utilize CPU and I/O resources with low task runtimes, we argue for
a pipelined task execution model where the execution framework manages I/O
requests for input, output and intermediate data. Finally, we discuss the
challenges in converting all jobs to use tiny tasks and propose changes to
enable this.

\eat{While smaller tasks are useful in existing frameworks, fully supporting tiny
tasks across the datacente has many challenges. }

\eat{However, tasks that read data
...
to build a file system for tiny tasks. }

\eat{Supporting tiny tasks also requires a
...
than $100\mu$s of overhead. execution model that ensures low task runtimes and 
full utilization of CPU and I/O resources.}
...
to optimize whether outputs are written to disk or kept in memory. 
