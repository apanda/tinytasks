...
in Figure~\ref{??}), supporting sub-second tasks for all jobs in large clusters
...
on response time. We find that tasks that complete in hundreds of milliseconds
...
JVM for each task; newer frameworks like Spark~\cite{??} reduce
overhead to $5$ms.
...
in realizing tiny tasks. For example, in Hadoop MapReduce, the smallest
